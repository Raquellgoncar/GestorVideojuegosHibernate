...
public class ActualizarVideojuegoVista extends JFrame {
    ...
    private JButton btnBuscar, btnLimpiarBusqueda, btnActualizar, btnAtras;
    private ResourceBundle texts;
    private ActualizarVideojuegoController controller;

    public ActualizarVideojuegoVista(Usuario usuario) { this(usuario, null); }

    public ActualizarVideojuegoVista(Usuario usuario, ModificarVideojuegosVista padre) {
    ...
    private void initComponents() {
        ...
        cabecera.setBorder(BorderFactory.createEmptyBorder(25, 40, 10, 40));
        ...
        btnBuscar = crearBotonMorado(texts.getString("update.search"), new Dimension(100, 34), e -> buscarVideojuegos());
        ...
        modelo = new DefaultTableModel(new Object[]{
                texts.getString("update.table.id"), texts.getString("update.table.title"),
                texts.getString("update.table.platform"), texts.getString("update.table.year"),
                texts.getString("update.table.rating"), texts.getString("update.table.favorite")
        ...
        panelTabla.setBorder(BorderFactory.createEmptyBorder(10, 40, 10, 40));
        ...
        btnAtras = crearBotonMorado(texts.getString("update.back"), new Dimension(120, 34), e -> volverAtras());
        btnActualizar = crearBotonMorado(texts.getString("update.update"), new Dimension(190, 40), e -> actualizarSeleccionado());
        JPanel pI = new JPanel(new FlowLayout(FlowLayout.LEFT)); pI.setOpaque(false); pI.add(btnAtras);
        JPanel pD = new JPanel(new FlowLayout(FlowLayout.RIGHT)); pD.setOpaque(false); pD.add(btnActualizar);
    ...
    private void cargarVideojuegos() {
        ...
        for (Videojuego v : controller.obtenerVideojuegosUsuario()) {
            modelo.addRow(new Object[]{v.getId(), v.getTitulo(), v.getPlataforma(),
                    v.getFechaMostrada(), v.getValoracionMostrada(),
                    v.isFavorito() ? texts.getString("update.yes") : texts.getString("update.no")});
        }
    }
    ...
    private void buscarVideojuegos() {
        ...
        modelo.setRowCount(0);
        for (Videojuego v : controller.obtenerVideojuegosUsuario()) {
            if (texto.isEmpty() || texto.equals(texts.getString("update.search.placeholder").toLowerCase())
    ...
    private void actualizarSeleccionado() {
        ...
        if (fila == -1) { JOptionPane.showMessageDialog(this, texts.getString("update.select.warning"),
                texts.getString("update.select.title"), JOptionPane.WARNING_MESSAGE); return; }
        int id = (int) modelo.getValueAt(fila, 0);
        Videojuego v = controller.obtenerVideojuegoPorId(id);
        new ActualizarVideojuegoDialog(this, controller.getUsuario(), v).setVisible(true);
        cargarVideojuegos();
    }
